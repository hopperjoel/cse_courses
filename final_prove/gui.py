...
import math
import number_entry as ne

#circle_area = math.pi * r**
#pendulum_swing = 2*math.pi * (square root of n divided by 9.81)
#area of rectangle = w * h
#volume of a tire = mat.pi * w** * a * (w * a + (2540 * d)) / 10000000
...
def main():
    ...
    frm_main.master.title("Some Calculation")
    frm_main.pack(padx=4, pady=3, fill=tk.BOTH, expand=1)

    # Call the populate_main_window function, which will add
    # labels, text entry boxes, and buttons to the main window.
    fill_main_window(frm_main)

    # Start the tkinter loop that processes user events
    # such as key presses and mouse button clicks.
    root.mainloop()

def fill_main_window(frm_main):
    ...
    lbl_calc= tk.Label(frm_main, text="")

    # Create a integer entry box where the user will enter her age.
    ent_age = ne.IntEntry(frm_main, 1, 90, width=5)

    # Create a label that displays "Rates:"
    lbl_ = tk.Label(frm_main, text="Rates:")
